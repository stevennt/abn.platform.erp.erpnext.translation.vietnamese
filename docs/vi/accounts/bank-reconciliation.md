...
Bạn có thể sử dụng Plaid (xem [trang Tích hợp Plaid](/docs/v13/user/manual/en/erpnext_integration/plaid_integration)) để tự động đồng bộ hóa tài khoản ngân hàng của bạn với ERPNext. Tất cả các giao dịch ngân hàng của bạn sẽ được tự động nhập vào ERPNext.
