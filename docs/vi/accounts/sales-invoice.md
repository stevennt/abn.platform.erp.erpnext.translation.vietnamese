...
![SO Flow](https://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/wwwhttps://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/www/docs/v13/assets/img/accounts/so-flow.png)
...
1. Giá sẽ được lấy tự động nếu [Item Price](../stock/item-price.md) được thêm vào, nếu không hãy thêm giá vào bảng.
...
1. Lưu và Xác nhận.
 ![New Sales Invoice](https://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/wwwhttps://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/www/docs/v13/assets/img/accounts/new-sales-invoice.png)
...
![POS and Credit Note Options](https://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/wwwhttps://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/www/docs/v13/assets/img/accounts/pos-and-credit-note-in-sales-invoice.png)

Dành cho Ấn Độ:
**e-Way Bill No**: Theo quy định của GST, đơn vị vận chuyển cần mang theo e-Way Bill. Để biết cách tạo e-Way Bill, [hãy truy cập trang này](../regional/india/auto-generate-e-way-bill-JSON.md).
...
* **Hạn thanh toán (Due Date)**: Ngày mà khoản thanh toán đến hạn (nếu bạn bán hàng trả chậm). Hạn mức tín dụng có thể được thiết lập từ danh mục [Khách hàng](../CRM/customer.md#24-credit-limit-and-payment-terms).
...
![Chi tiết Đơn mua hàng](https://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/wwwhttps://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/www/docs/v13/assets/img/accounts/purchase-order-details-in-invoice.png)

### 3.4 Địa chỉ và Liên hệ

* **Địa chỉ Khách hàng:** Đây là Địa chỉ thanh toán của Khách hàng.
* **Người liên hệ**: Nếu Khách hàng là một công ty, người cần liên hệ sẽ được lấy từ biểu mẫu [Khách hàng](../CRM/customer.md) nếu đã được thiết lập.
* **Khu vực:** [Khu vực](../selling/territory.md) là vùng mà Khách hàng thuộc về, được lấy từ biểu mẫu Khách hàng. Giá trị mặc định là Tất cả khu vực.
...
* Chọn tiền tệ của Khách hàng chỉ để tham chiếu cho Khách hàng, trong khi việc hạch toán kế toán sẽ chỉ được thực hiện bằng tiền tệ cơ sở của Công ty. Tìm hiểu thêm [tại đây](articles/managing-transactions-in-multiple-currency.md).
* Duy trì tài khoản phải thu riêng biệt bằng tiền tệ của Khách hàng. Khoản phải thu cho hóa đơn này sẽ được hạch toán bằng chính loại tiền tệ đó. Đọc [Kế toán đa tiền tệ](multi-currency-accounting.md) để tìm hiểu thêm.

### 3.6 Bảng giá

Nếu bạn chọn một Bảng giá, thì giá của mặt hàng sẽ được lấy từ bảng đó. Việc tích vào 'Bỏ qua Quy tắc định giá' sẽ bỏ qua các [Quy tắc định giá](pricing-rule.md) được thiết lập trong Kế toán > Quy tắc định giá.

Đọc [tài liệu Bảng giá](../stock/price-lists.md) để biết thêm.


### 3.7 Bảng Mặt hàng

> Lưu ý: Từ phiên bản 13 trở đi, chúng tôi đã giới thiệu sổ cái bất biến (immutable ledger), thay đổi các quy tắc về việc hủy phiếu kho và hạch toán các giao dịch kho lùi ngày trong ERPNext. [Tìm hiểu thêm tại đây](articles/immutable-ledger-in-erpnext.md).
...
* **quét mã vạch**: Bạn có thể thêm các Mặt hàng vào bảng Mặt hàng bằng cách quét mã vạch của chúng nếu bạn có máy quét mã vạch. Đọc tài liệu về [theo dõi mặt hàng bằng mã vạch](../stock/articles/track-items-using-barcode.md) để biết thêm.

* Mã mặt hàng, tên, mô tả, Hình ảnh và Nhà sản xuất sẽ được lấy từ [danh mục Mặt hàng](../stock/item.md).

* **Chiết khấu và Biên lợi nhuận**: Bạn có thể áp dụng chiết khấu cho từng Mặt hàng theo tỷ lệ phần trăm hoặc trên tổng số tiền của Mặt hàng. Đọc [Áp dụng Chiết khấu](../selling/articles/applying-discount.md) để biết thêm chi tiết.

* **Đơn giá**: Đơn giá sẽ được lấy nếu đã được thiết lập trong [Bảng giá](../stock/price-lists.md) và Tổng số tiền sẽ được tính toán.

* **Giao hàng thả (Drop Ship)**: Drop Shipping là khi bạn thực hiện giao dịch bán hàng, nhưng Mặt hàng lại được giao bởi Nhà cung cấp. Để biết thêm, hãy truy cập trang [Giao hàng thả](../selling/articles/drop-shipping.md).

* **Chi tiết kế toán**: Các tài khoản Thu nhập và Chi phí có thể được thay đổi tại đây nếu bạn muốn. Nếu Mặt hàng này là một [Tài sản](../asset/asset.md), nó có thể được liên kết tại đây. Điều này hữu ích khi bạn [bán một Tài sản](../asset/selling-an-asset.md).

* **Doanh thu hoãn lại**: Nếu doanh thu cho Mặt hàng này sẽ được lập hóa đơn thành nhiều phần trong các tháng tới, hãy tích vào 'Bật Doanh thu hoãn lại'. Để biết thêm, hãy truy cập [trang Doanh thu hoãn lại](deferred-revenue.md).
...
* **Số Lô và Số serial**: Nếu Mặt hàng của bạn có số serial hoặc theo lô, bạn sẽ phải nhập [Số serial](../stock/serial-no.md) và [Lô hàng](../stock/batch.md) trong bảng Mặt hàng. Bạn được phép nhập nhiều Số serial trong một dòng (mỗi số trên một dòng riêng biệt) và bạn phải nhập cùng một số lượng Số serial.
## 4. Thêm
### Tác động kế toán
...
* **Có:** Thuế (các khoản nợ phải trả cho chính phủ)
 ...
 ![SI Ledger](https://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/wwwhttps://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/www/docs/v13/assets/img/accounts/ledger-updates-on-sales-invoice-submission.png)
...
1. [Thuế theo từng mặt hàng](item-tax-template.md)
