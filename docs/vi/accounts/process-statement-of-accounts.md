...
1. Công cụ sử dụng ID email của khách hàng để gửi báo cáo cho họ. Nếu không tìm thấy các mục email dưới đây trong Danh bạ Khách hàng, công cụ sẽ không cho phép bạn chọn Khách hàng tương ứng, vì vậy vui lòng đảm bảo các chi tiết sau đã được điền trong tài liệu Khách hàng.
    ...
    - Email Thanh toán của Khách hàng: Đây là mục bắt buộc và có thể được thiết lập trong [Danh bạ Khách hàng](/docs/v13/user/manual/en/CRM/contact#1-how-to-create-a-contact) với tùy chọn "Is Billing Contact" được tích chọn.
    - Email Chính của Khách hàng: Mục này không bắt buộc, trừ khi bạn chọn "Send To Primary Contact" trong biểu mẫu.

2. Thiết lập Tài khoản Email với tính năng gửi thư đi (outgoing) đã được bật. Tìm hiểu thêm về điều này [tại đây](/docs/v13/user/manual/en/setting-up/email/email-account).
...
3. Thiết lập các bộ lọc Sổ cái cho các bản đối chiếu sẽ được gửi cho khách hàng.
    ...
    - "Dự án" và "Trung tâm chi phí" là các trường [Table MultiSelect](/docs/v13/user/manual/en/customize-erpnext/articles/table-multiselect-field). Nghĩa là bạn có thể chọn nhiều Dự án và Trung tâm chi phí trong các bộ lọc Sổ cái.
...
Bạn có thể sử dụng các thẻ Jinja để nhập các giá trị động từ:

- Khách hàng mà email sẽ được gửi đến dưới đối tượng "customer"
- Bất kỳ trường nào trong tài liệu Xử lý Đối chiếu Công nợ đã chọn dưới đối tượng "doc"
- Bất kỳ phương thức nào trong [`frappe.utils`](https://github.com/frappe/frappe/bl