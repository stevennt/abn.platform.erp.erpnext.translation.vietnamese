...
9. Sau khi lưu, một Bên tiêu dùng sự kiện (Event Consumer) sẽ được tạo trên trang web bên sản xuất (http://test_site_producer:8000). Trong quy trình này, các khóa của người dùng trên trang web người tiêu dùng sẽ tự động được sao chép vào tài liệu Bên tiêu dùng sự kiện trên trang web bên sản xuất.
    ...
    ![Event Producer](https://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/wwwhttps://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/www/docs/v13/assets/img/automation/event-producer-doc.png)
...
4. Lưu.
    ...
    ![Event Consumer](https://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/wwwhttps://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/www/docs/v13/assets/img/automation/event-consumer-doc.png)
...
3. Bây giờ, hãy tạo một Bên sản xuất sự kiện trên tài khoản được lưu trữ và đặt URL của bên sản xuất thành URL của tài khoản cục bộ của bạn.
4. Thêm bất kỳ DocType nào bạn muốn đồng bộ vào bảng con DocType của Bên sản xuất sự kiện.
5. Phê duyệt các DocType.

## 3. Các tính năng


### 3.1 Hủy đăng ký các bản cập nhật

Với tư cách là Bên tiêu dùng sự kiện, nếu bạn muốn hủy đăng ký các bản cập nhật cho bất kỳ DocType nào bạn đã đăng ký trước đó, hãy tích vào ô hủy đăng ký (unsubscribe) tương ứng với DocType đó. Bạn sẽ không nhận được thêm bất kỳ bản cập nhật nào từ trang web bên sản xuất cho DocType cụ thể đó sau khi bạn đã hủy đăng ký.

![Unsubscribe](https://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/wwwhttps://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/www/docs/v13/assets/img/automation/unsubscribe-event.png)

### 3.2 Nhật ký cập nhật sự kiện (Event Update Log)
"Nhật ký cập nhật sự kiện" ghi lại mọi hành động tạo, cập nhật và xóa cho các tài liệu có bên tiêu dùng trên trang web Bên sản xuất sự kiện.
Để xem Nhật ký cập nhật sự kiện:

Đi tới: **Trang chủ > Tự động hóa > Truyền dữ liệu sự kiện > Nhật ký cập nhật sự kiện (Event Update Log)**.

- Đối với loại 'Tạo' (Create), hãy xem Loại cập nhật, Loại tài liệu, Tài liệu