...
* **To Bill**: Chưa được lập [Hóa đơn bán hàng](/docs/v13/user/manual/en/accounts/sales-invoice).
...
[Contacts](/docs/v13/user/manual/en/CRM/contact) và [Addresses](/docs/v13/user/manual/en/CRM/address) được lưu trữ riêng biệt để bạn có thể đính kèm nhiều Liên hệ hoặc Địa chỉ cho khách hàng.
...
### 3.5 Bảng Mặt hàng
* **Barcode**: Bạn có thể theo dõi các Mặt hàng bằng cách sử dụng [mã vạch](/docs/v13/user/manual/en/stock/articles/track-items-using-barcode).

* Mã Mặt hàng, tên, mô tả, Hình ảnh và Nhà sản xuất sẽ được lấy từ [danh mục Mặt hàng](/docs/v13/user/manual/en/stock/item).

* **Scan Barcode**: Bạn có thể thêm các Mặt hàng vào bảng Mặt hàng bằng cách quét mã vạch nếu bạn có máy quét mã vạch. Đọc tài liệu về [theo dõi mặt hàng bằng mã vạch](/docs/v13/user/manual/en/stock/articles/track-items-using-barcode) để biết thêm.

* **Discount và Margin**: Bạn có thể áp dụng chiết khấu cho từng Mặt hàng theo tỷ lệ phần trăm hoặc trên tổng số tiền của Mặt hàng đó. Đọc [Applying Discount](/docs/v13/user/manual/en/selling/articles/applying-discount) để biết thêm chi tiết.

* **Rate**: Đơn giá được lấy nếu đã được thiết lập trong [Bảng giá](/docs/v13/user/manual/en/stock/price-lists) và tổng Số tiền được tính toán.

* **Item Tax Template**: Bạn có thể thiết lập một Mẫu thuế Mặt hàng để áp dụng một mức Thuế cụ thể cho Mặt hàng này. Để biết thêm, hãy truy cập [trang này](/docs/v13/user/manual/en/accounts/item-tax-template).
...
* **Warehouse và Reference**: Kho nơi các Mặt hàng được gửi đến Khách hàng sẽ được hiển thị. Ngoài ra, một Đơn bán hàng cũng sẽ được hiển thị nếu Phiếu giao hàng này được tạo theo luồng: 'Sales Order > Delivery Note'.

* **Batch No và Serial No**: Nếu Mặt hàng của bạn được quản lý theo số lô hoặc số serial, bạn sẽ phải nhập [Số serial](/docs/v13/user/manual/en/