<!-- add-breadcrumbs -->
# Mẫu Dự án

**Mẫu Dự án là một chuỗi các nhiệm vụ được xác định trước và được sắp xếp theo một lịch trình quy định.**

Các mẫu này có thể được áp dụng cho các loại Dự án tương tự và các nhiệm vụ trong đó sẽ được tự động điền khi tạo mỗi dự án mới.

Để truy cập Mẫu Dự án, hãy đi đến:

> Home > Projects > Projects > Project Template

<img class="screenshot" alt="Project Template" src="https://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/wwwhttps://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/www/docs/v13/assets/img/project/projects-project-template.png">

## 1. Cách tạo Mẫu Dự án mới:

  1. Đi đến Danh sách Mẫu Dự án và nhấn vào New.
  2. Thêm các chi tiết sau:
      * **Project Template Name**: Tiêu đề của Mẫu Dự án.
      * **Project Type**: Các Mẫu Dự án, tương tự như dự án có thể được phân loại thành các loại dự án khác nhau, ví dụ: Nội bộ hoặc Bên ngoài.
      * **Tasks**: Mỗi Mẫu Dự án sẽ có một bộ các nhiệm vụ được sắp xếp theo trình tự xác định trước. Trong bảng này, bạn có thể chọn các nhiệm vụ mà bạn muốn cho mẫu này.

## 2. Các chủ đề liên quan
  1. [Project](project.md)
  2. [Task](tasks.md)

{next}