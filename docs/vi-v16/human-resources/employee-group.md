<!-- add-breadcrumbs -->

# Nhóm nhân viên

**Nhóm nhân viên là sự phân nhóm các Nhân viên dựa trên một số thuộc tính như Chức danh, Cấp bậc, Chi nhánh, v.v.**

Để truy cập Nhóm nhân viên, hãy đi tới:

> Home > Human Resources > Employee > Employee Group

## 1. Điều kiện tiên quyết

Trước khi tạo Nhóm nhân viên, bạn nên tạo các tài liệu sau:

* [Nhân viên](/docs/v16/user/manual/vi/human-resources/employee)


## 2. Cách tạo Nhóm nhân viên

1. Đi tới danh sách Nhóm nhân viên, nhấn vào New.
1. Nhập Tên.
1. Chọn và thêm ID Nhân viên vào nhóm. Tên Nhân viên sẽ được tự động lấy về.
1. Lưu.

    <img class="screenshot" alt="Employee Group" src="https://raw.githubusercontent.com/frappe/erpnext_documentation/master/erpnext_documentation/www/docs/v13/assets/img/human-resources/employee-group.png">

## 3. Các tính năng

### 3.1 Thỏa thuận mức độ dịch vụ

Một Nhóm nhân viên có thể được thêm vào DocType [Thỏa thuận mức độ dịch vụ](/docs/v16/user/manual/vi/support/service-level-agreement), nơi Mức độ dịch vụ có thể được chỉ định cho một Nhóm nhân viên cụ thể.


## 4. Các chủ đề liên quan

1. [Loại hình nhân viên](/docs/v16/user/manual/vi/human-resources/employment-type)
1. [Chi nhánh](/docs/v16/user/manual/vi/human-resources/branch)
1. [Phòng ban](/docs/v16/user/manual/vi/human-resources/department)
1. [Chức danh](/docs/v16/user/manual/vi/human-resources/designation)
1. [Cấp bậc nhân viên](/docs/v16/user/manual/vi/human-resources/employee-grade)