...
2. Tài sản cố định như nội thất, máy tính, v.v.
3. Khoản phải thu (AR) tức là các hóa đơn chưa thanh toán mà bạn đã gửi cho Khách hàng.
4. Tài sản lưu động như số dư ngân hàng, tiền mặt tại quỹ, tiền gửi, v.v.

### Nợ phải trả

1. Tài khoản vốn như vốn của cổ đông (hoặc chủ sở hữu).
2. Nợ ngắn hạn như các khoản vay, lương phải trả, v.v.
3. Khoản phải trả (AP) tức là các hóa đơn chưa thanh toán mà Nhà cung cấp đã gửi cho bạn.
...
Trong phiên bản ERPNext v16, bạn có thể sử dụng tính năng **Xem trước Sổ cái (Ledger Preview)** trước khi **Xác nhận (Submit)** các bút toán để đảm bảo tính chính xác. Các bút toán đầu kỳ có thể được tạo bằng Công cụ tạo Hóa đơn đầu kỳ hoặc thông qua [Bút toán](journal-entry.md).

> **Lưu ý:** Bút toán đầu kỳ chỉ dành cho các tài khoản Bảng cân đối kế toán và không dành cho các tài khoản Báo cáo kết quả hoạt động kinh doanh.
...
Với các tính năng mới trong v16, bạn có thể kiểm tra tính nhất quán thông qua:
* **Bảng cân đối thử hợp nhất (Consolidated Trial Balance)** để xem tổng thể các công ty.
* **Mẫu báo cáo tài chính (Financial Report Templates)** để đối chiếu số dư đầu kỳ với các báo cáo đã đóng từ hệ thống cũ.
* Kiểm tra việc **Tự động chốt tồn kho (Automatic Closing Stock)** để đảm bảo giá trị hàng tồn kho được chuyển sang chính xác.
