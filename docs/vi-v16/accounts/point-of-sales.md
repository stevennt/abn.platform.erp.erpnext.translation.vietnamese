...
Trong ERPNext v16, Hóa đơn bán hàng có thể được tạo từ POS. Hệ thống đã được cải tiến với các tính năng kế toán mới như: Mẫu báo cáo tài chính (Financial Report Templates), Bảng cân đối số phát sinh hợp nhất (Consolidated Trial Balance), tách biệt Giá vốn hàng bán (COGS) với Chi phí dịch vụ (Service Expenses), Xem trước sổ cái (Ledger Preview) trước khi Xác nhận, Tự động tính giá trị tồn kho (Automatic Closing Stock) và Kế toán định kỳ (Periodic Accounting).
...
1. Lưu ý rằng bạn cần phải có Mặt hàng trong Kho trước khi có thể bán. Nếu Mặt hàng không có sẵn, một dấu chấm đỏ sẽ hiển thị bên cạnh Mặt hàng khi được chọn.
  ![Màn hình POS](https://docs.erpnext.com/docs/v16/assets/img/accounts/pos-screen.png)
1. Khi tất cả các Mặt hàng đã được thêm vào, hãy nhấp vào Thanh toán (Pay). Bạn sẽ được yêu cầu Xác nhận Hóa đơn bán hàng.
1. Chọn phương thức thanh toán, Xác nhận (Submit). Bạn có thể sử dụng tính năng **Xem trước sổ cái (Ledger Preview)** để kiểm tra các bút toán trước khi thực hiện Xác nhận.
1. Sau đó, bạn có thể in hóa đơn POS.
  ![Thanh toán POS](https://docs.erpnext.com/docs/v16/assets/img/accounts/pos-checkout.gif)
...
1. Chọn hàng trong giỏ hàng và nhấp vào nút 'Remove' (Xóa) trong bàn phím số.
  ...
  ![Loại bỏ Mặt hàng khỏi POS](https://docs.erpnext.com/docs/v16/assets/img/accounts/remove-item-from-pos.png)
...
POS tính toán số tiền thừa mà khách hàng đã trả, người dùng có thể trả lại từ tài khoản tiền mặt. Người dùng phải thiết lập tài khoản cho số tiền thừa trong hồ sơ POS.
  ...
  ![Thay đổi số tiền thừa trong POS](https://docs.erpnext.com/docs/v16/assets/img/accounts/change-amount-in-pos.png)
...
![Thêm Khách hàng mới trong POS](https://docs.erpnext.com/docs/v16/assets/img/accounts/pos-add-new-customer.gif)
...
![Gửi Email kèm Biên lai POS](https://docs.erpnext.com/docs/v16/assets/img/accounts/pos-email.png)
...
![Nhập đóng POS](https://docs.erpnext.com/docs/v16/assets/img/accounts/pos-closing-entry.png)

Nhập số tiền đã thu được cho mỗi phương thức thanh toán. Nếu bạn nhận thấy có bất kỳ sự khác biệt nào giữa số tiền trên hệ thống và số tiền mặt thực tế thu được, hãy tạo một Bút toán chênh lệch (Difference Posting).
